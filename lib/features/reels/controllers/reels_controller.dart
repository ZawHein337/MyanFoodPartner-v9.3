...
class ReelsController extends GetxController implements GetxService {
  ...
  Future<ReelDetailsModel?> getReelDetails(int reelId) async {
    return await reelServiceInterface.getReelDetails(reelId);
  }
  ...
  Future<void> submitReel({
    ...
    if (isAlwaysVisible) {
      ...
      String startDate = DateConverter.stringToMDY(dateRange.start.toString());
      String endDate = DateConverter.stringToMDY(dateRange.end.toString());
      body['dates'] = '$startDate - $endDate';
    }

    body['order_now_button'] = orderNowButton ? '1' : '0';
    if(foodId != null){
      body['food_id'] = foodId.toString();
    }
    ...
    if (response.statusCode == 200) {
      ...
      showCustomSnackBar('content_size_to_high'.tr);
    }
    else {
      // because sometime the backend return html error when we fetch error
      try{
