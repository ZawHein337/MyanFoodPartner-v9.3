...
class ReelRepository implements ReelRepositoryInterface {
  ...
  Future<ReelDetailsModel?> getReelDetails(int reelId) async {
    final Response response = await apiClient.getData('${AppConstants.reelDetailsUri}?reel_id=$reelId');
    if (response.statusCode == 200) {
      return ReelDetailsModel.fromJson(response.body);
    }
    return null;
  }
  ...
  Future<Response> updateReel(Map<String, String> body, List<MultipartBody> files) async {
    return await apiClient.postMultipartData(AppConstants.updateReelUri, body, files, [],  handleError: false);
  }
