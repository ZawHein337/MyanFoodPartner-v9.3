  ...
  Widget build(BuildContext context) {
    return GetBuilder<AdvertisementController>(
      builder: (adsController) {
        ...
        bool _hasNothingChanged() {
          final ads = widget.adsDetailsModel!;

          if (titleController[0].text.trim() != (ads.title ?? '')) return false;

          if (descriptionController[0].text.trim() != (ads.description ?? '')) return false;

          final originalDate = "${DateConverter.stringToMDY(ads.startDate ?? '')} - ${DateConverter.stringToMDY(ads.endDate ?? '')}";
          if (validationController!.text != originalDate) return false;

          if (adsController.selectedAdsType != (ads.addType ?? '')) return false;

          if (adsController.isReviewChecked != (ads.isReviewActive == 1)) return false;
          if (adsController.isRatingsChecked != (ads.isRatingActive == 1)) return false;

          if (adsController.pickedVideoFile != null) return false;
          if (adsController.pickedProfileImage != null) return false;
          if (adsController.pickedCoverImage != null) return false;
                ...
                Expanded(
                  child: CustomButtonWidget(
                    ...
                    onPressed: (){
                      ...
                      if (widget.adsDetailsModel != null && !widget.fromCopy!) {
                        if (_hasNothingChanged()) {
                          Get.back();
                          return;
                        }
                      }
