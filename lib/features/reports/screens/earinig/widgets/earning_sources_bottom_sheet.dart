        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
                ...
                children: [
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Text('${isEarning ? '+' : '-'} ${PriceConverter.priceFormate(double.tryParse(breakdown.values.toList()[index].toString()) ?? 0.0, currencySymbolDirectionRight: false)}', style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeSmall)),
                    ]),
                  ),

                  if (index != (reportController.getEarningReportModel?.transactions!.data.length ?? 0) - 1) const SizedBox(height: Dimensions.paddingSizeSmall),
                ],
              );
            }),
            ...
            if(!isEarning && (breakdown == null || breakdown.isEmpty)) Container(
              ...
              child: Row(children: [
                Text(isExpense ? (transactionModel.badge ?? transactionModel.reference ?? '') : 'subscription_fee'.tr, style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeSmall)),
                const Spacer(),

                Text('${'-'} ${PriceConverter.priceFormate((transactionModel.amount ??  0.0), currencySymbolDirectionRight: false)}', style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeSmall)),
              ]),
            ),
